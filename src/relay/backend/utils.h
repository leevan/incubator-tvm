...
inline const CallNode* GetRootCall(const CallNode* current_call, int depth,
  ...
  CHECK(current_call);
  CHECK(depth >= 0) << "depth:" << depth;
  CHECK(static_cast<size_t>(depth) < expected_op_names.size()) << "depth:" << depth << "size:" << expected_op_names.size();
  CHECK(IsOp(current_call, expected_op_names[depth])) << "expect:" << expected_op_names[depth];
